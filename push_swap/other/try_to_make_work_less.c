...
void radix_sort(Stack *a, Stack *b, int size) {
    normalize(a, size); // Add normalization
    int max_num = size - 1;
    int max_bits = 0;
    ...
    for (int i = 0; i < max_bits; i++) {
        ...
        for (int j = 0; j < n; j++) {
            int num = peek(a);
            if (((num >> i) & 1) == 1) {
                ra(a); // Keep in A by rotating
                count_kept++;
            } else {
                pb(a, b); // Push to B
            }
        }
        // Optimize rotations using rra if it saves steps
        if (count_kept > 0 && count_kept < a->size) {
            int reverse_rotations = a->size - count_kept;
            for (int k = 0; k < reverse_rotations; k++) {
                rra(a);
            }
        }
        // Bring all elements back to A from B
        while (!isEmpty(b)) {
            pa(a, b);
        }
    }
